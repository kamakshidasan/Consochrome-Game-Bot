...
array_input = [input_1,input_2,input_3,input_4,input_5,input_6,input_7,input_8]
array_output = [output_1,output_2,output_3,output_4,output_5,output_6,output_7,output_8]
...
whitearray = {0: 'white', 1: 'white', 2: 'white', 3: 'white', 4: 'white', 5: 'white', 6: 'white', 7: 'white'}
...
def function():
	...
	while True:
		if time.time() > timeout:
			...
			newarray = {}
			for i in xrange(0,8):
				newarray[i] = get_colour_name(im.getpixel(array_output[i]))
			change1 = DictDiffer(newarray, array)
			change2 = DictDiffer(newarray, whitearray)
			array = newarray
			first_list = list(change1.changed())
			second_list = list(change2.changed())
			if(len(first_list) == 0 or len(second_list) == 0):
				...
				for i in first_list:
					...
					if(newarray[i]!='white'):
						try:
							#print inputcolours[newarray[i]]
							value = inputcolours[newarray[i]]
						except KeyError:
							temparray = []
							for colour in inputcolours.keys():
								temparray.append(name_to_rgb(colour))
								tree = sp.KDTree(temparray)
								distance, result = tree.query(name_to_rgb(newarray[i]))
								value = inputcolours[rgb_to_name(temparray[result])]
						#print array_input[value]
						mousePos(array_input[value])
						leftClick()
						count += 1
						print count
						if(count < 80):
...
def changeInput(im):
	inputcolours = {}
	for i in xrange(0,8):
		inputcolours[get_colour_name(im.getpixel(array_input[i]))] = i

	if 'darkred' in inputcolours:
		inputcolours['brown'] = inputcolours['darkred']
	elif 'brown' in inputcolours:
		inputcolours['darkred'] = inputcolours['brown']


	if 'forestgreen' in inputcolours:
		inputcolours['limegreen'] = inputcolours['forestgreen']
	elif 'limegreen' in inputcolours:
		inputcolours['forestgreen'] = inputcolours['limegreen']

	if 'darkslategray' in inputcolours:
		inputcolours['darkcyan'] = inputcolours['darkslategray']

	if 'dimgrey' in inputcolours:
		inputcolours['darkcyan'] = inputcolours['dimgrey']
	return inputcolours
